...
const Invoice = () => {
  ...
  useEffect(() => {
    ...
    if (settingsActiveTab === "invoice" && invoiceActiveTab) {
      dispatch(
        updateSettingsStoredKey({
          key: `hs_msbc_settings_${settingsActiveTab}_${invoiceActiveTab}`,
        })
      );
    }
    ...
    if (settingsActiveTab === "invoice") {
      if (invoiceActiveTab === "syncRules") {
        ...
        if (pipelinePreference && dealStagePreference && invoicePreference) {
          dispatch(
            updateSettingsStoredKey({
              key: `hs_msbc_settings_${settingsActiveTab}_${invoiceActiveTab}_btns`,
            })
          );
          dispatch(
            updateGuideSteps({
        ...
        if (pipelinePreference && !dealStagePreference) {
          dispatch(
            updateSettingsStoredKey({
              key: `hs_msbc_settings_${settingsActiveTab}_${invoiceActiveTab}_dealStage`,
            })
          );
        ...
        if (pipelinePreference && dealStagePreference && !invoicePreference) {
          dispatch(
            updateSettingsStoredKey({
              key: `hs_msbc_settings_${settingsActiveTab}_${invoiceActiveTab}_generate`,
            })
          );
          dispatch(
            updateGuideSteps({
